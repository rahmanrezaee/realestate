...
final mainUrlSite = 'http://badam.af/';
...
Future getPostsHttp({String type = ""}) async {
  try {
    ...
    final reponse = await http.get(
        mainUrlSite + "wp-json/wp/v2/posts" + type,
...
Future getMainSlider() async {
  try {
    ...
    final reponse = await http.get(
        mainUrlSite + "wp-json/wp/v2/slide",
        headers: {"Accept": "application/json"});
...
Future removetoFavorite(userId,propertyId) async {
  try {
    final reponse = await http.post(
      mainUrlSite +
          "wp-json/badam/v1/removeFavorite",
          body: {
            "user_id":userId,
            "property_id":propertyId
          },
...
Future submitPropertytoServer(String token, Property item, imageId,location) async {
  try {
...
Future deletePropertyServer(token, id) async {
  ...
  await http.delete(baseUrl,
      headers: {"Authorization": "Bearer $token", 'accept': '*/*'}).then((data){
        print(data);
      });
}
